-- Create the table
CREATE TABLE pmd.hmi_m_45s
(
   recnum bigint CONSTRAINT hmi_m_45s_pkey PRIMARY KEY,
   sunum bigint,
   slotnum integer,
   segment text,
...
-- Create the indexes
CREATE UNIQUE INDEX hmi_m_45s_t_rec_index_camera_idx
  ON hmi_m_45s
  USING btree
  (t_rec_index, camera)
TABLESPACE indexspace;

CREATE INDEX hmi_m_45s_date_obs_idx
  ON hmi_m_45s
  USING btree
  (date_obs)
TABLESPACE indexspace;
...
CREATE OR REPLACE FUNCTION hmi.populate_hmi_m_45s() RETURNS TRIGGER AS $populate_hmi_m_45s$
BEGIN
   -- First we try an update. It will fail if the row didn't exist yet.
   UPDATE pmd.hmi_m_45s
   SET
      recnum=NEW.recnum,
      sunum=NEW.sunum,
      slotnum=NEW.slotnum,
   ...
   INSERT INTO pmd.hmi_m_45s (recnum, sunum, slotnum, segment, date_obs, wavelnth, quality, t_rec_index, camera)
   SELECT
      NEW.recnum,
      NEW.sunum,
      NEW.slotnum,
   ...
   RETURN NEW;
END;
$populate_hmi_m_45s$ LANGUAGE plpgsql;


-- Create the trigger
CREATE TRIGGER trigger_populate_hmi_m_45s
AFTER INSERT ON hmi.m_45s
FOR EACH ROW
EXECUTE PROCEDURE hmi.populate_hmi_m_45s();

-- Fill up the table
INSERT INTO pmd.hmi_m_45s
(
   recnum,
   sunum,
   slotnum,
