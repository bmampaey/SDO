...
def call_vso_sum_alloc(data_series_name, sunum, size, vso_sum_alloc = "vso_sum_alloc", log = logging):
	...
	log.debug("Command \"%s\" output: %s", " ".join(command), output)
	...
	log.debug("vso_sum_alloc sudir: %s", sudir)
	
	return sudir
	
	# Call vso_sum_put to register the directory in sum_main
	output = subprocess.check_output()
	log.debug("vso_sum_put output:%s", output)
