...
from PMD.models import DataSites, DataLocationRequest

# In the JMD, the len(sunums) < 100 et il chuffle les sunums si il y a error
def get_download(download_request_url, sunums, timeout = 120, max_attempts = 3, method = "GET"):
	'''Request a data site for the download of sunums'''
	
	# We parse the URL
	url_parts = urlparse.urlparse(download_request_url)
	if url_parts.scheme != "http" and url_parts.scheme != "":
		raise Exception("URL scheme is not http. URL was %s" % download_request_url)
	elif not url_parts.netloc:
		raise Exception("URL netloc not specified. URL was %s" % download_request_url)
	
	result = dict()
	if len(sunums) > 0:
		# We add the sunums to the URL
		sunums = [str(sunum) for sunum in sunums]
		sunums_request = "?op=exp_su&method=url_quick&format=json&formatvar=dataobj&protocol=as-is&requestid=NOASYNCREQUEST&sunum={sunums}".format(sunums=",".join(sunums))
		...
		while remaining_attempts > 0 and response_content is None:
			try:
				...
				connection.request(method, url_parts.path + sunums_request)
				...
				logging.warning("Failed attempt %s/%s retrieving URL %s: %s.", max_attempts-remaining_attempts, max_attempts, download_request_url + sunums_request, str(why))
			finally:
				connection.close()
		
		
		if remaining_attempts == 0 or response_content is None: # We failed getting a response from the server
			raise Exception("Could not retrieve URL %s, %s failures. Last failure was %s." % (download_request_url + sunums_request, max_attempts, str(why)))
			...
			try:
				...
				raise Exception("Error parsing json from response to URL %s: %s. Response was: %s" % (download_request_url + sunums_request, str(why), response_content))
			
			if not "data" in response_json: # The response is not what we expected
				raise Exception("No data member available in response to URL %s. Response was: %s" % (download_request_url + sunums_request, response_content))
...
#TODO check how to add path and size as a attribute of a request
def download_request_thread(request_queue, output_queue, data_site, delay = 5):
	'''Accumulate data download requests and call get_download to get the paths'''
	...
	while True:
		...
		# We gather available requests until we have enough
		while total_priority < data_site.download_request_max_priority and len(sunums) < data_site.download_request_max_size and not request_queue.empty():
			request = request_queue.get() # We should never block because we just checked that the queue is not empty and are the only reader 
			sunums.append(request.sunum)
			total_priority += request.priority
			requests[request.sunum] = request
		
		if not sunums:
			logging.debug("No request available, sleeping for %s seconds", delay)
			time.sleep(delay)
			continue
		
		# We make the actual download request
		try:
			results = get_download(data_site.download_request_url, sunums, data_site.download_request_timeout, max_attempts = data_site.download_request_max_attempts)
		except Exception, why: # It failed :( We still return the requests but without the paths
			log.error(str(why))
			for request in requests.itervalues():
				request.path = None
				output_queue.put((request, None))
		else:
			# We update the requests with the path and the size
			for sunum, request in requests.iteritems():
				request.path = None
				request.size = 0
				if sunum not in results or results[sunum] is None:
					...
					# Do we need those additional checks ?
						...
						# Because we don't know how to cope we just yell
						log.error("Path %s found for sunum %s, but sunum in result %s is different than request sunum %s", result['path'], sunum, result['sunum'], request.sunum)
					
					elif 'series'in result and result['series'] != str(request.data_series):
						# Because we don't know how to cope we just yell
						log.error("Path %s found for sunum %s, but series in result %s is different than request data series %s", result['path'], sunum, result['series'], request.data_series)
				...
				# We send the updated request down the pipe
				output_queue.put(request) 

def manage_request_results(request_queue, output_queue):
	'''Update the data download tables with the results and forward them'''
	while True:
		request = request_queue.get()
		# Add the path to the data site table
		if request.path:
			data_download = request.data_site.data_download_model(data_series = request.data_series, sunum = request.sunum, slotnum = request.slotnum, path = request.path)
			logging.debug("Saving path for request %s to data download table of %s", unicode(request), request.data_site.name)
			try:
				data_download.save()
			except IntegrityError, why:
				logging.debug("Path for request %s was already in table, updating")
				data_download = request.data_site.data_download_model.objects.get(data_series = request.data_series, sunum = request.sunum, slotnum = request.slotnum)
				data_download.path = request.path
				data_download.save()
		
		# Remove the request from the table
		logging.debug("Deleting data download request %s from data base", unicode(request))
		request.delete()
		if output_queue:
			output_queue.put(request)


def download_request_manager(request_queue, output_queue):
	'''Dispatch the data download requests to the corresponding download_request_thread'''
	request_queues = dict()
	
	# data download requests have a lt operator defined so that higher priorities will be first
	threads_output_queue = Queue.PriorityQueue()
	
	# We start a thread to manage the output from the threads
	thread = threading.Thread(group=None, name="manage_request_results", target=manage_request_results, args=(threads_output_queue, output_queue))
	thread.daemon = True
	thread.start()
	
	# We wait for requests and dispatch them to the threads
	while True:
		request = request_queue.get()
		if request.data_site.name not in request_queues:
			# We start 1 thread for the data site
			request_queues[request.data_site.name] = Queue.PriorityQueue()
			logging.debug("Starting a new thread download_request_thread for data site %s", request.data_site.name)
			thread = threading.Thread(group=None, name="download_request for %s" % request.data_site.name, target=download_request_thread, args=(request_queues[request.data_site.name], threads_output_queue, data_sites[request.data_site.name]))
			thread.daemon = True
			thread.start()
		
		# We send the request to the good thread
		request_queues[request.data_site.name].put(request)
...
if __name__ == '__main__':
	...
	thread = threading.Thread(group=None, name="download_request_manager", target=download_request_manager, args=(request_queue, output_queue))
	thread.daemon = True
	thread.start()
	number_requests = 0
	for download_request in DownloadRequest.objects.all():
		request_queue.put(download_request)
		number_requests += 1
	
	while number_requests:
		request = output_queue.get()
		logging.info("Request %s with priority %s has been treated", request, request.priority)
		number_requests -= 1
...
#	sunums = [547696328, 547696327]
#	request_url = "http://sdo-db1.cfa.harvard.edu/cgi-bin/VSO/DRMS/vso_jsoc_fetch.cgi"
#	request_url = "http://jsoc.stanford.edu/cgi-bin/ajax/jsoc_fetch_VSO"
#	logging.info("Requesting info at url %s for sunums %s", request_url, sunums)
#	try:
#		result = get_download(request_url, sunums, 10, 1)
#	except Exception, why:
